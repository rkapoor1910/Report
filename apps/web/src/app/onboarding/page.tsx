...
// ─── Quick-start templates (Ess Gee style use cases) ────────────────────────
        ...
        <div className="w-full max-w-xl">
          ...
          {step === 'welcome' && (
            <div className="text-center space-y-6">
              ...
              <div className="bg-muted/30 border border-border rounded-xl p-5 text-left space-y-3">
                {[
                  { emoji: '🔌', text: 'Connect a data source — email, spreadsheet, database, or app' },
                  { emoji: '📊', text: 'Choose which reports to monitor and map your columns' },
                  { emoji: '📲', text: 'Get plain-English alerts with stats, red flags, and actions' },
                ].map(item => (
              ...
              <p className="text-xs text-muted-foreground">
                You're on the <span className="font-medium capitalize">{plan}</span> plan — 14-day free trial, no credit card needed
              </p>
            </div>
          )}
          ...
          {step === 'connector' && (
            <div className="space-y-6">
              ...
              <div className="space-y-2">
                {[
                  { icon: '📧', label: 'Gmail / Outlook',    desc: 'Pick up Excel/CSV reports from email attachments', href: '/dashboard/connectors/new?type=gmail' },
                  { icon: '🗂️', label: 'Google Sheets',      desc: 'Sync live from a shared spreadsheet',              href: '/dashboard/connectors/new?type=google_sheets' },
                  { icon: '🧾', label: 'Tally',              desc: 'Connect directly to your Tally installation',      href: '/dashboard/connectors/new?type=tally' },
                  { icon: '📊', label: 'Upload Excel / CSV', desc: 'Manually upload a report file to start',           href: '/dashboard/connectors/new?type=excel_upload' },
                ].map(option => (
                  <a
                    key={option.label}
                    href={option.href}
          ...
          {step === 'report' && (
            <div className="space-y-6">
              ...
              <div className="space-y-2">
                {[
                  { emoji: '📦', label: 'Stock / inventory',     desc: 'Know when stock is low or sitting idle' },
                  { emoji: '🛍️', label: 'Retailer sell-out',    desc: 'Track daily sales by store, brand, SKU' },
                  { emoji: '🎯', label: 'Target vs actual',      desc: 'Get alerted when the team misses target' },
                  { emoji: '🚚', label: 'Dispatch & logistics',  desc: 'Monitor orders shipped, pending, returned' },
                  { emoji: '↩️', label: 'Returns & defects',    desc: 'Spot return rate spikes before they escalate' },
                ].map(option => (
                  <a
                    key={option.label}
          ...
          {step === 'alert' && (
            <div className="space-y-6">
              <div>
                ...
                <p className="text-sm text-muted-foreground mt-1">You can change this anytime — start with the channel you check most</p>
              </div>
              <div className="space-y-2">
              ...
              >
                {saving
                  ? <><Loader2 size={14} className="animate-spin" /> Setting up your account…</>
